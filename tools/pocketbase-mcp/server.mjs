...
const expectedFields = {
  courses: ["title", "description", "startDate", "endDate", "status", "students", "teachers", "classes", "assignments", "inquiries"],
  classes: ["title", "description", "date"],
  assignments: ["title", "description", "dueDate"],
  links: ["title", "url", "file", "type", "class", "assignment"],
  deliveries: ["assignment", "student", "repositoryUrl", "file", "grade", "feedback", "verdict", "status"],
  delivery_attempts: [
    "delivery",
    "assignment",
    "student",
  ...
  inquiries: ["title", "description", "status", "author", "course", "class", "assignment"],
  inquiry_responses: ["inquiry", "author", "content"],
  enrollment_requests: ["firstName", "lastName", "dni", "birthDate", "email", "phone", "courses", "status"],
  account_requests: ["firstName", "lastName", "dni", "googleEmail", "status"],
};
...
const tools = [
  {
    name: "health",
    description: "Check whether the configured Epixum Datastack PocketBase instance is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
  ...
  {
    name: "whoami",
    description: "Show current PocketBase auth state without exposing tokens or passwords.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
  ...
  {
    name: "validate_datastack_schema",
    description: "Check whether the PocketBase schema has the collections and fields expected by this project.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
...
const toolHandlers = {
  ...
  async validate_datastack_schema() {
    const data = await adminApi("/api/collections?perPage=500&sort=name");
    ...
    const fields = Object.entries(expectedFields).map(([collectionName, fieldNames]) => {
      ...
      return {
        ...
        expected: fieldNames,
        present: fieldNames.filter((fieldName) => presentFields.has(fieldName)),
        missing: fieldNames.filter((fieldName) => !presentFields.has(fieldName)),
      };
    });

    return {
      ok: missingCollections.length === 0 && fields.every((result) => result.missing.length === 0),
      url: PB_URL,
      expectedCollections,
      missingCollections,
      fields,
  ...
  switch (message.method) {
    case "initialize":
      return {
        ...
        serverInfo: {
          name: "pocketbase-epixum-datastack",
          version: "0.1.0",
        },
      };
    ...
    case "resources/list":
      return {
        resources: [
          {
            uri: "pocketbase://epixum-datastack/connection",
            name: "Epixum Datastack PocketBase connection",
            description: "Current PocketBase URL and sanitized auth state.",
            mimeType: "application/json",
          },
          {
            uri: "pocketbase://epixum-datastack/schema",
            name: "Epixum Datastack schema status",
    ...
    case "resources/read":
      if (message.params?.uri === "pocketbase://epixum-datastack/connection") {
        return {
          contents: [
            {
              uri: "pocketbase://epixum-datastack/connection",
      ...
      if (message.params?.uri === "pocketbase://epixum-datastack/schema") {
        return {
          contents: [
            {
              uri: "pocketbase://epixum-datastack/schema",
              mimeType: "application/json",
              text: JSON.stringify(await toolHandlers.validate_datastack_schema(), null, 2),
            },
          ],
        };
      }
...
async function adminApi(requestPath, options = {}) {
  const pb = await authenticate();
  ...
  const response = await fetch(`${PB_URL}${requestPath}`, {
    ...options,
    headers,
  });
...
async function getPocketBase() {
  ...
  try {
    ...
    throw rpcError(
      -32000,
      `Could not load the PocketBase SDK. Run "npm install" in the project before using this MCP. Detail: ${error.message}`,
    );
  }
}
