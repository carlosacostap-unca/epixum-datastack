...
export default async function EstudiantesLayout({
  children,
}: {
  ...
  const avatarUrl = user.avatar 
    ...
    : `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || "Estudiante")}&background=3fff8b&color=0e0e0e`;
      ...
      <header className="sticky top-0 z-50 w-full bg-[var(--color-surface-variant)]/40 backdrop-blur-[20px] border-b border-[var(--color-outline-variant)]/50 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8 md:gap-12">
            <Link href="/estudiantes" className="flex items-center gap-3 group">
              ...
              <span className="font-headline font-bold text-lg hidden sm:block tracking-tight">Portal Estudiante</span>
            </Link>

            <nav className="hidden md:flex items-center gap-8">
              <Link href="/estudiantes" className="text-sm font-bold text-[var(--color-on-surface-variant)] hover:text-[var(--color-primary)] transition-colors flex items-center gap-2">
        ...
        {/* Mobile Navigation (Bottom Bar or secondary top bar) */}
        <div className="md:hidden flex items-center justify-around px-4 py-3 border-t border-[var(--color-outline-variant)]/30 bg-[var(--color-surface-variant)]/60">
          <Link href="/estudiantes" className="flex flex-col items-center gap-1 text-[var(--color-on-surface-variant)] hover:text-[var(--color-primary)] transition-colors">
            <span className="material-symbols-outlined text-[24px]">local_library</span>
            <span className="text-[10px] font-bold uppercase tracking-wider">Mis Cursos</span>
          </Link>
        </div>
      </header>
