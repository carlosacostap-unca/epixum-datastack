      ...
      <header className="mb-24 flex flex-col md:flex-row gap-20 items-start justify-between">
        <div className="max-w-3xl">
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ml-4 ${
          ...
          <h1 className="text-6xl font-headline tracking-tight text-[var(--color-on-surface)] mb-6 leading-tight">
            {course.title}
          </h1>
          {course.description && (
            <p className="text-[var(--color-on-surface-variant)] text-xl leading-relaxed" dangerouslySetInnerHTML={{ __html: course.description }} />
          )}
        </div>

        {/* Floating Stats Card */}
        <div className="bg-[var(--color-surface-container-low)] rounded-[2.5rem] p-8 min-w-[280px] relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.3)] flex flex-col gap-8 border border-[var(--color-outline-variant)]">
            ...
            <div className="flex flex-col gap-6">
              {classes.map((c) => (
                <Link 
                  ...
                  key={c.id} 
                  className="bg-[var(--color-surface-container-low)] hover:bg-[var(--color-surface-container)] transition-colors rounded-[2rem] p-8 border border-[var(--color-outline-variant)] flex flex-col md:flex-row md:items-center justify-between gap-6 group"
                >
                  <div>
                  ...
                  <span className="px-6 py-3 bg-[var(--color-surface-container-highest)] text-[var(--color-on-surface)] rounded-full group-hover:text-[var(--color-primary)] group-hover:bg-[var(--color-surface-container-high)] transition-colors font-bold text-sm whitespace-nowrap flex items-center gap-2">
                    <span>Gestionar</span>
