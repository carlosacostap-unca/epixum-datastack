...
export default async function ClassesPage({ params }: ClassesPageProps) {
  ...
  // Ordenar clases por fecha (más recientes primero, o como se prefiera)
        ...
        <header className="mb-12 max-w-6xl mx-auto flex justify-between items-end">
          <div>
            <div className="flex items-center gap-2 text-on-surface-variant mb-4">
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest text-indigo-400">Clases</span>
            </div>
            <h1 className="text-[36px] font-headline font-medium tracking-tight text-on-surface mb-2">Clases</h1>
            <p className="text-on-surface-variant font-body text-lg">Gestiona las clases y sesiones de este curso.</p>
          </div>
          <div className="flex gap-4 mb-2">
            <Link href={`/courses/${course.id}/classes/new`} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-all text-sm font-semibold shadow-lg shadow-indigo-500/20">
          ...
          {sortedClasses.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {sortedClasses.map((cls) => (
                <section key={cls.id} className="glass-panel rounded-xl p-6 flex flex-col hover:border-white/10 transition-colors">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-headline font-medium text-on-surface line-clamp-1">{cls.title}</h3>
                        <p className="text-xs font-bold uppercase tracking-widest text-indigo-400 mt-1">
                          {cls.date ? new Date(cls.date).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : 'Fecha sin especificar'}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="text-sm text-slate-400 line-clamp-3 mb-6 flex-1" dangerouslySetInnerHTML={{ __html: cls.description || 'Sin descripción' }}></div>
                  
                  <div className="pt-4 border-t border-white/5 flex justify-between items-center mt-auto">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-slate-500 text-sm">link</span>
                      <span className="text-xs text-slate-400">{cls.expand?.links?.length || 0} recursos adjuntos</span>
                    </div>
                    <Link href={`/courses/${course.id}/classes/${cls.id}`} className="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors flex items-center gap-1">
                      Ver detalles
            ...
            <section className="glass-panel rounded-xl p-12 flex flex-col items-center justify-center text-center">
              <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4">
                <span className="material-symbols-outlined text-3xl text-slate-400">video_library</span>
              </div>
              <h3 className="text-xl font-headline font-medium mb-2">No hay clases disponibles</h3>
              <p className="text-slate-400 mb-6 max-w-md">Todavía no se han agregado clases a este curso. Puedes empezar creando la primera sesión.</p>
              <Link href={`/courses/${course.id}/classes/new`} className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all text-sm font-bold shadow-lg shadow-indigo-500/20">
                <span className="material-symbols-outlined text-sm">add</span>
                Crear Primera Clase
              </Link>
            </section>
